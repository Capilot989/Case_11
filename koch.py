...
def get_color(step: int, total: int) -> tuple:
    ...
    Creates a linear gradient where blue decreases and red increases
    proportionally to the current step in the total progression.
    ...
    Returns:
        tuple: RGB color as (red, green, blue) tuple with values 0-255
    """
...
def snowflake(size: int, depth: int, SF_total: int) -> None:
    """
       ...
       Args:
           ...
           SF_total (int): Total segments for color gradient in snowflake
       """
    for _ in range(3):
        koch(size, depth, SF_total)
        t.right(120)
