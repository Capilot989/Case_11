...
def count_segments(depth: int) -> int:
    ...
    return count_segments(depth - 1) * 4


def ice_fract(depth: int, size: float, total: int) -> None:
    """
    Draw an ice fractal using recursion.

    This function recursively draws a fractal pattern that resembles ice crystals
    or snowflakes by repeatedly dividing the line segments 
    and adding perpendicular branches.
    ...
    if depth == 0:
        ...
        ice_fract(depth-1, size/4, total)
        turtle.left(90)

        ice_fract(depth-1, size/6, total)
        turtle.right(180)
