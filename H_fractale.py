...
def draw_H(x: float, y: float, width: int, height: int,
    ...
    Args:
        x (float): X-coordinate of the H's origin
        y (float): Y-coordinate of the H's origin
        width (float): Width of the H shape
        height (float): Height of the H shape
        step(int): Current position in the color transition sequence
        total_steps(int): total numb of steps
...
def H_recursive(x: float, y: float, width: int, height: int,
                depth: int, step: int, total_steps: int) -> None:
    """
    Recursively draw H-fractal pattern.
    This function draws an H-shape and recursively calls itself 
    for four quadrants around the H's endpoints.
    ...
    draw_H(x, y, width, height, step, total_steps)
    step += 1
    ...
    step = H_recursive(x, y + height / 2, new_width,
                       new_height, new_depth, step, total_steps)
    step = H_recursive(x + width / 2, y + height / 2, new_width,
                       new_height, new_depth, step, total_steps)
    step = H_recursive(x, y, new_width, new_height,
                       new_depth, step, total_steps)
    step = H_recursive(x + width / 2, y, new_width,
                       new_height, new_depth, step, total_steps)
