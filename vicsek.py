...
def get_color(step: int, total: int) -> tuple:
    """
       ...
       Creates a gradient where blue intensity decreases linearly while
       red intensity increases proportionally to the current step.

       Args:
           step (int): Current position in the color progression
...
def draw_square(x: float, y: float, size: int, color) -> None:
    """
        ...
        Args:
            ...
            color: Color tuple or string for the square outline
        """
    t.pencolor(color)
    t.penup()
    t.goto(x, y)
...
def vicsek(x: float, y: float, size: int, depth: int, total: int) -> None:
    ...
    for px, py in positions:
        vicsek(px, py, new_size, depth - 1, total)
